...
int main( int argc, char* args[] )
{
    ...
    while( quit == false )
    {
        ...
        ciclo++;
        frame++;
         int xs = rand() % (640+1); // Número aleatorio para la posición en x de las pelotitas
         int ys = rand()%(480+1); // Número aleatorio en y para la posición de las pelotitas.
         int vs=rand()%(3)+1; // Velocidad en la que se movilizarán.
         int vs2=rand()%(3)+1; // Velocidad 2

    //Rango entre aparición de pelotitas en pantalla, cada 200 iteraciones.

    if(ciclo%200==0)
    {
        l.agregar(new Pelotita(xs,ys,vs2,vs,pelotita_azul));
        l.agregar(new Pelotita(xs,ys,vs,vs2,pelotita_roja));
    }
        click_x = -1;
        click_y = -1;
